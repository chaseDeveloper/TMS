...
function openUpdWindow(rowData) {
	
	var usrId = rowData.USR_ID;
	
    
	$.ajax({
        url: "<%=request.getContextPath() %>/adminPool/getPoolInfo.do", // 서버 URL 수정 필요
        type: "post",
        data: { usrId: usrId }, // 서버에 넘길 데이터
        dataType: "json",
...
function fillDialogWithData(data) {
	...
	function fn_getPoolInfo() {
		...
		$.ajax({
			url:"<%=request.getContextPath() %>/admin/getPoolInfo.do",
			type:"post",
			cache: false,
			data:{},			
			dataType : "json",
			success:function(data){
				if(data.result == "S") {
					...
					if (poolFulltimeTeacherList && poolFulltimeTeacherList.length > 0) {
					 ...
					 console.log	(poolFulltimeTeacherList);
